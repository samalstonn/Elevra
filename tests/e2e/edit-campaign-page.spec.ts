...
test("edit campaign page", async ({ page, candidate: _candidate }) => {
  ...
  const modifiedBlocks = blocks.filter(
    (block) =>
      ...
      block.body?.includes("This is a test") ||
      (block.type === "LIST" &&
        block.items?.some((item) => item.includes("This is a test"))) ||
      (block.type === "IMAGE" && block.imageUrl?.includes("Elevra.png"))
  );

  expect(modifiedBlocks.length).toBe(5); // There are 5 modified blocks (header, description, sub-header, list item, image)
  ...
  expect(unmodifiedBlocks.length).toBe(elevraStarterTemplate.length - 5); // There are 5 unmodified blocks
