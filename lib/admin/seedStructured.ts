...
export type SeedStructuredOptions = {
...
export type SeedStructuredResult = {
  hidden: boolean;
...
export async function seedStructuredData({
  ...
  const hiddenFlag = hiddenOverride ?? forceHidden ?? hiddenInProd;
  ...
  return { hidden: hiddenFlag, results };
}
...
function coerceType(t: string | undefined) {
  const v = (t || "").toUpperCase();
  if (v === "STATE" || v === "UNIVERSITY" || v === "NATIONAL") return v;
  return "LOCAL";
}
