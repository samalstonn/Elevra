...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch election links" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof candidateId !== "number" || typeof electionId !== "number") {
      return NextResponse.json({ error: "candidateId and electionId must be numbers" }, { status: 400 });
    }
    const data = {
      candidateId,
      electionId,
    ...
    return NextResponse.json({ error: "Failed to create election link" }, { status: 500 });
  }
}
